...
if __name__ == "__main__":
    ...
    content_table_fetcher = GlueDataFetcher("raw", "raw_content", "latest")
    content = content_table_fetcher.get_dataframe()
    
    # As done in Raw EDA, DQ to drop NULL values from newsLanguage & author
    content_stg = content.na.drop(subset=["newsLanguage", "author"])
    
    # Saving data frame as staged table_name
    dest_s3_location = 's3://shivam-trial-s3/staging/content/'
    glue_table_manager = GlueTableManager('stage', 'stage_content', content_stg ,dest_s3_location)
